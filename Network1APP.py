...
if (len(selected_onus_acct)==0 and len(selected_offus_acct)==0):
  ...
  df_node_fraud['label'] = df_node_fraud['name'].apply(lambda x: 'onus Orig' if x in selected_onus_acct else 'offus Orig')
  ...
  if taskRadio == '1. Transaction(s) involved selected subject(s) only':
    ...
    nx.set_node_attributes(G1, df_node_fraud.set_index('name')['label'].to_dict(), 'label')
