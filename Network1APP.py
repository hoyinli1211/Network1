...
if (len(selected_onus_acct)==0 and len(selected_offus_acct)==0):
  ...
  secondlayer_acct = pd.concat([df_edge_firstlayer['Orig'], df_edge_firstlayer['Dest']], axis=0).drop_duplicates().rename('name')
  ...
  thirdlayer_acct = pd.concat([df_edge_secondlayer['Orig'], df_edge_secondlayer['Dest']], axis=0).drop_duplicates().rename('name')
